...
class RoverControllerMain:
    ...
    def navigate(self, rover_id, x=0, y=0, yaw=0):
        ...
        self.logger.info(f"Начало отправки команды роверу {rover_id} -> ({x}, {y}, {yaw})")
        ...
        if not self.send_command(rover_id, f"POSE:{current_pos['x']},{current_pos['y']},{current_pos['yaw']}"):
            return False
